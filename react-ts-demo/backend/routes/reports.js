...
router.get("/daily/:userId", async (req, res) => {
  try {
    ...
    const dailyMasteredWords = JSON.parse(
...
router.get("/monthly/:userId", async (req, res) => {
  try {
    ...
    const avgAccuracy = accuracyCount > 0 ? totalAccuracy / accuracyCount : 0;
    const avgDailyTime =
      activeDays > 0 ? Math.round(totalStudyTime / activeDays) : 0;
    ...
    const report = {
      ...
      achievements,
      suggestions,
      summary: `本月学习${activeDays}天，累计${totalStudyTime}分钟，平均每日${avgDailyMinutes}分钟，掌握${totalNewWords}个新单词`,
    };

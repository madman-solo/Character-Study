...
import type { ChangeWordModalProps, ChildWord } from '../../../types/vocabulary';
import { fetchChildWordsByLetter, filterWordsByLength } from '../../../services/childVocabularyService';
...
const ChangeWordModal = ({
  ...
  const loadWords = async () => {
    setIsLoading(true);
    try {
      const fetchedWords = await fetchChildWordsByLetter(selectedLetter, grade, 100);
  ...
  return (
    <div className="child-change-word-modal-overlay" onClick={onClose}>
      <div className="child-change-word-modal" onClick={(e) => e.stopPropagation()}>
